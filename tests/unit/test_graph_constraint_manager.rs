...
fn test_constraint_config_with_options() {
    ...
    assert_eq!(constraint.options.get("case_sensitive"), Some(&json!(false)));
...
async fn test_create_constraint_graph_not_found() {
    ...
    let result = adapter.create_constraint("non_existent_graph", constraint).await;
...
async fn test_constraint_lifecycle() {
    ...
    // Create constraint
    let create_result = adapter.create_constraint(graph_id, constraint.clone()).await;
...
async fn test_different_constraint_types() {
    ...
    for constraint in &constraints {
        let result = adapter.create_constraint(graph_id, constraint.clone()).await;
        assert!(result.is_ok(), "Failed to create constraint: {}", constraint.name);
    }
...
async fn test_constraint_already_exists_error() {
    ...
    let first_result = adapter.create_constraint(graph_id, constraint.clone()).await;
...
async fn test_drop_nonexistent_constraint() {
    ...
    let result = adapter.drop_constraint(graph_id, "nonexistent_constraint").await;
    assert!(result.is_err());
    assert!(result
        .unwrap_err()
        .to_string()
...
async fn test_composite_unique_constraint() {
    ...
    let result = adapter.create_constraint(graph_id, composite_constraint.clone()).await;
    assert!(result.is_ok());
