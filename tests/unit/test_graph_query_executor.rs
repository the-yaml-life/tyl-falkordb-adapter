...
async fn test_execute_read_query_basic() {
    ...
    assert_eq!(query_result.metadata.get("is_write_query"), Some(&json!(false)));
    assert_eq!(query_result.metadata.get("original_query"), Some(&json!(query.query)));
}
...
async fn test_execute_write_query_basic() {
    ...
    // Verify metadata content
    assert_eq!(query_result.metadata.get("is_write_query"), Some(&json!(true)));
    assert_eq!(query_result.metadata.get("original_query"), Some(&json!(query.query)));
}
...
async fn test_execute_query_with_parameters() {
    ...
    let query = GraphQuery::write("CREATE (n:Person {name: $name, age: $age}) RETURN n".to_string())
    ...
    assert_eq!(query_result.metadata.get("parameter_count"), Some(&json!(2)));
}
...
async fn test_execute_query_auto_routing() {
    ...
    assert_eq!(read_result.metadata.get("is_write_query"), Some(&json!(false)));
    ...
    assert_eq!(write_result.metadata.get("is_write_query"), Some(&json!(true)));
}
...
async fn test_complex_parameterized_query() {
    ...
    let query = GraphQuery::write("CREATE (p:Person {name: $name, age: $age, active: $active, score: $score})".to_string())
    ...
    assert!(result.is_ok());
    let query_result = result.unwrap();
    assert_eq!(query_result.metadata.get("parameter_count"), Some(&json!(4)));
}
...
async fn test_query_result_metadata_completeness() {
    ...
    assert_eq!(query_result.metadata.get("original_query"), Some(&json!(query.query)));
    assert_eq!(query_result.metadata.get("is_write_query"), Some(&json!(false)));
    assert_eq!(query_result.metadata.get("parameter_count"), Some(&json!(1)));
    
    // Verify executed_at is a valid RFC3339 timestamp
    let timestamp = query_result.metadata.get("executed_at").unwrap().as_str().unwrap();
...
async fn test_query_with_multiple_parameters() {
    ...
    let query = GraphQuery::read("MATCH (p:Person) WHERE p.name IN [$name1, $name2] AND p.age >= $min_age RETURN p".to_string())
    ...
    let query_result = result.unwrap();
    assert_eq!(query_result.metadata.get("parameter_count"), Some(&json!(params.len())));
}
