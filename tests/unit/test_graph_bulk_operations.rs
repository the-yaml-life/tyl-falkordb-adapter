...
use tyl_falkordb_adapter::{
    FalkorDBAdapter, GraphInfo, GraphBulkOperations, MultiGraphManager,
};
use tyl_graph_port::{
    BulkOperation, GraphNode, GraphRelationship,
};
...
fn create_test_nodes(count: usize) -> Vec<GraphNode> {
    ...
    for i in 0..count {
        ...
        node.properties.insert("name".to_string(), json!(format!("Node {}", i)));
...
fn create_test_relationships(count: usize) -> Vec<GraphRelationship> {
    ...
    for i in 0..count {
        ...
        rel.properties.insert("weight".to_string(), json!(0.5 + i as f64 * 0.1));
        rel.properties.insert("created_batch".to_string(), json!(true));
...
async fn test_bulk_create_nodes_nonexistent_graph() {
    ...
    let result = adapter.bulk_create_nodes("non_existent_graph", bulk_op).await;
...
async fn test_export_data_graphml() {
    ...
    let result = adapter.export_data(graph_id, "graphml", export_params).await;
