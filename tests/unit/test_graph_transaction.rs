...
async fn test_read_only_transaction_context() {
    ...
    let mut context =
        TransactionContext::read_only().with_isolation(IsolationLevel::RepeatableRead);
...
async fn test_begin_transaction_graph_not_found() {
    ...
    let result = adapter
        .begin_transaction("non_existent_graph", context)
        .await;
...
async fn test_transaction_not_found_operations() {
    ...
    assert!(commit_result.unwrap_err().to_string().contains("not found"));

    // Try to rollback non-existent transaction
    let rollback_result = adapter
        .rollback_transaction(graph_id, non_existent_tx)
        .await;
...
async fn test_concurrent_transactions() {
    ...
    let tx1_result = adapter
        .begin_transaction(graph_id, tx1_context.clone())
        .await;
    let tx2_result = adapter
        .begin_transaction(graph_id, tx2_context.clone())
        .await;
    ...
    let _ = adapter
        .rollback_transaction(graph_id, &tx2_context.id)
        .await;
}
...
async fn test_isolation_levels() {
    ...
    for level in isolation_levels {
        let context = TransactionContext::new().with_isolation(level.clone());
