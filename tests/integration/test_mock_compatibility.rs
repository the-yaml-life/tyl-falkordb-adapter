...
use serde_json::json;
use tyl_falkordb_adapter::{FalkorDBAdapter, GraphInfo, MultiGraphManager, GraphStore};
use tyl_graph_port::{MockGraphStore, GraphNode, GraphRelationship};
...
async fn test_node_operations_compatibility() {
    ...
    test_node.properties.insert("name".to_string(), json!("Test Node"));
...
async fn test_relationship_operations_compatibility() {
    ...
    test_rel.properties.insert("since".to_string(), json!("2023"));
    
    // Test relationship creation with mock
    let mock_create_result = mock_store.create_relationship(graph_id, test_rel.clone()).await;
